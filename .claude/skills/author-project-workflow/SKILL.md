...
Use when asked to add a custom Tier-2 workflow for a project (anything beyond agentops.json + built-ins).
...
5. Add entry to `agentops.json` with `"schedule": "continuous"` (or cron); omit `taskQueue` unless you need a non-default queue (`proj-<project>` is the default).
