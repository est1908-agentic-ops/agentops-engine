...
- A Grafana host + login for the same cluster (conventionally `grafana.<same base domain>`). Grafana requires auth here — if you don't have credentials, ask for them; never write real ones into this skill, code, commits, or memory. Use them for the session, then let them go.
