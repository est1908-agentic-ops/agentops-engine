...
describe('PR landing contracts', () => {
  ...
  it('accepts child handoff and standalone inputs', () => {
    ...
    expect(
      PrLandingInputSchema.safeParse({
        ...base,
        workspace: { workspaceRef: '/ws/t', branch: 'agentops/t', validatedHeadSha: 'abc' },
      }).success,
    ).toBe(true);
    expect(
      PrLandingInputSchema.safeParse({ ...base, agentCreated: false, headBranch: 'feature/x' })
        .success,
    ).toBe(true);
  });
  ...
  it('requires exact typed snapshot and merge results', () => {
    ...
    expect(
      MergePrResultSchema.parse({ kind: 'merged', headSha: 'abc', mergeCommitSha: 'def' }).kind,
    ).toBe('merged');
  });
