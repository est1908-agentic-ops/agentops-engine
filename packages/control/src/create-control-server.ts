...
export interface ControlDeps {
  ...
  // with any of these three unset the routes return 503. The control ingress
  // is protected by chart-managed Traefik basicAuth (issue #4).
...
async function dispatch(deps: ControlDeps, req: IncomingMessage): Promise<HandlerResponse | undefined> {
  ...
  if (pathname === '/api/tiers') {
    ...
    // fleet-mutating writes. The control ingress is protected by chart-managed
    // Traefik basicAuth (issue #4).
    if (req.method === 'PUT') {
