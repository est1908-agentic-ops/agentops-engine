...
describe('reconcileAgents', () => {
  ...
  it('deletes orphans not in the manifest', () => {
    const plan = reconcileAgents(
      [],
      [
        {
          id: scheduleId('p', 'gone'),
          scheduleSpec: '0 2 * * *',
          workflow: 'whiteboxBugHunt',
          paused: false,
        },
  ...
  it('pauses a disabled agent and resumes a re-enabled one', () => {
    ...
    expect(
      reconcileAgents(
        [spec({ name: 'a', enabled: false })],
        [{ id: idA, scheduleSpec: '0 2 * * *', workflow: 'whiteboxBugHunt', paused: false }],
        'p',
      ).toPause,
    ).toEqual([idA]);
    expect(
      reconcileAgents(
        [spec({ name: 'a', enabled: true })],
        [{ id: idA, scheduleSpec: '0 2 * * *', workflow: 'whiteboxBugHunt', paused: true }],
        'p',
      ).toResume,
    ).toEqual([idA]);
  });
  ...
  it('re-points a project workflow with no explicit taskQueue from ENGINE_QUEUE to proj-<project>', () => {
    ...
    expect(reconcileAgents(declared as any, existing, 'acme').toUpdate.map((s) => s.name)).toContain(
      'scan',
    );
  });
});
...
describe('resolveAgentQueue', () => {
  it('honors an explicit taskQueue above all', () => {
    expect(resolveAgentQueue({ workflow: 'whiteboxBugHunt', taskQueue: 'custom' }, 'acme')).toBe(
      'custom',
    );
    expect(resolveAgentQueue({ workflow: 'rollbarMonitor', taskQueue: 'custom' }, 'acme')).toBe(
      'custom',
    );
  });
  it('routes a built-in workflow to the engine queue', () => {
    expect(resolveAgentQueue({ workflow: 'whiteboxBugHunt', taskQueue: undefined }, 'acme')).toBe(
      ENGINE_QUEUE,
    );
    expect(
      resolveAgentQueue(
        { workflow: 'devCycle', taskQueue: undefined },
        'acme',
        LEGACY_ENGINE_QUEUE,
      ),
    ).toBe(LEGACY_ENGINE_QUEUE);
  });
  it('routes a project (Tier-2) workflow to proj-<project>', () => {
    expect(resolveAgentQueue({ workflow: 'rollbarMonitor', taskQueue: undefined }, 'acme')).toBe(
      'proj-acme',
    );
  ...
  it('slugifies an operator-chosen project name so the queue is k8s/Temporal-safe', () => {
    ...
    expect(
      resolveAgentQueue(
        { workflow: 'rollbarMonitor', taskQueue: undefined },
        'Artem private agents',
      ),
    ).toBe('proj-artem-private-agents');
  });
});
...
it('excludes a disabled continuous agent (treated as terminate)', () => {
  const plan = reconcileContinuous(
    [{ ...cont('mon'), enabled: false }],
    [scheduleId('acme', 'mon')],
    'acme',
  );
  expect(plan.toTerminate).toEqual([scheduleId('acme', 'mon')]);
