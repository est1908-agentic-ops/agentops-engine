...
import { isRateLimitMessage, isSessionLimitMessage } from './provider-rate-limit';
...
describe('isRateLimitMessage', () => {
  ...
  it('matches a generic 429 that mentions rate limiting', () => {
    expect(isRateLimitMessage('429 Too Many Requests: rate limit exceeded, retry later')).toBe(true);
  });

  it('does not match a 429 with no rate-limit wording', () => {
    expect(isRateLimitMessage('429 payment required to continue using this model')).toBe(false);
  });

  it('does not match rate-limit wording with no 429', () => {
    expect(isRateLimitMessage('the request frequency for this endpoint is limited')).toBe(false);
  });
});
