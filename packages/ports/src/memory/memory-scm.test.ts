...
describe('MemoryScmPort', () => {
  it('opens a PR and returns an incrementing prRef', async () => {
    ...
    const first = await scm.openPr({
      repo: 'demo/repo',
      branch: 'agentops/t1',
      title: 't1',
      body: 'b',
    });
    const second = await scm.openPr({
      repo: 'demo/repo',
      branch: 'agentops/t2',
      title: 't2',
      body: 'b',
    ...
    scm.scriptFeedback('pr-1', [
      {
        ciStatus: 'failed',
        unresolvedThreads: 1,
        comments: [{ id: 'c1', body: 'fix', resolved: false }],
      },
  ...
  it('push accepts and ignores repo/workspaceRef (real git happens in the real adapter, not here)', async () => {
    const scm = new MemoryScmPort();
    await expect(
      scm.push('demo/repo', '/some/workspace/path', 'branch-x', 'hash-x'),
  ...
  it('scripts snapshots and merges at the exact expected head SHA', async () => {
    ...
    await expect(scm.mergePr({ prRef: 'pr-1', expectedHeadSha: 'abc' })).resolves.toEqual({
      kind: 'merged',
      headSha: 'abc',
      mergeCommitSha: 'merge-abc',
    });
    expect(scm.getOperations()).toContainEqual({
      type: 'mergePr',
      prRef: 'pr-1',
      expectedHeadSha: 'abc',
    });
    await expect(scm.getPrSnapshot('pr-1')).resolves.toMatchObject({
      state: 'merged',
      mergedHeadSha: 'abc',
    });
  });
  ...
  it('returns head-changed when the expected SHA does not match', async () => {
    ...
    await expect(scm.mergePr({ prRef: 'pr-1', expectedHeadSha: 'def' })).resolves.toEqual({
      kind: 'head-changed',
    });
