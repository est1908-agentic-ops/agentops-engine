...
- **gh**: an account with access to the engine/platform repos, plus one with access to each
  managed project's repo. If multiple gh accounts are logged in, the active one can flip
  mid-session — re-assert with `gh auth switch --user <acct>` right before each gh call
  (a flip shows up as spurious 404s).
...
SID=agent:<project>:smoke-test

temporal schedule describe --schedule-id "$SID" --namespace "$NS"   # unpaused, the project's smoke workflow, proj-<project>
temporal schedule trigger  --schedule-id "$SID" --namespace "$NS"   # fire now (ad-hoc action)
temporal workflow list --namespace "$NS" --query 'WorkflowType="<smokeWorkflow>"' --order-by-start-time desc | head
...
```sh
temporal workflow start --type <smokeWorkflow> --task-queue proj-<project> \
  --workflow-id smoke-manual-1 --namespace "$NS" \
  --input '{"repo":"<owner>/<repo>","project":"<project>","message":"manual test"}'
temporal workflow result --workflow-id smoke-manual-1 --namespace "$NS"
```

> Scope note: a smoke workflow returning `{ok:true}` proves the **worker-serves-its-queue** half of
