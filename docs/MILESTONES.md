...
**Superseded (2026-07-09):** the chart-driven static project registry above was fully removed in favor of the DB-backed managed project registry (`managed_projects` table, [managed-project-registry-design.md](superpowers/specs/2026-07-08-managed-project-registry-design.md)) — `agentops-platform` no longer uses per-project chart values/secrets at all; every project (GitHub- or Linear-tracked) is registered via `engine project add` / Mission Control. See the [Linear trigger design's DB-only addendum](superpowers/specs/2026-07-09-linear-trigger-design.md).
