...
- [ ] **Step 1: Delete the files** listed above and remove their imports/wiring. Keep `control`'s read-only `GET /api/projects` + `/api/registry/repos` but repoint them at a `FileManagedProjectStore` (read-only; console stays a viewer per the spec).
- [ ] **Step 2: `pnpm -r typecheck && pnpm -r test`** — fix every dangling reference until green. Delete tests asserting removed behavior (CRUD 409s, decrypt round-trip, getparams route).
- [ ] **Step 3: Commit** `refactor: retire DB project registry, X25519 crypto, gateway plugin route, control CRUD`.
- [ ] **Step 4 (doc, not code): drop the table** — add a line to the engine's deploy notes / the platform `DEPLOY.md`: after cutover verifies healthy, `DROP TABLE managed_projects;` in `agentops_engine` (manual, once). Not scripted against the live cluster.
