...
- **The `docker` driver also auto-loads every build result into the daemon's local image store**, regardless of whether `push:` actually ran — unlike the previous `docker-container` driver, whose per-run builder (and its output) was simply discarded. Since `ci.yaml` tags every build with `${{ github.sha }}` unconditionally (PR and `main` alike), every `build-images` run now leaves a uniquely-tagged, non-dangling image sitting in the shared daemon. A plain `docker system prune` (even `-a`) won't remove these, since they're tagged, not dangling — the eventual pruning fast-follow above needs to explicitly delete old SHA-tagged images (e.g. `docker image prune -a --filter "until=336h"`), not just prune the build cache.
