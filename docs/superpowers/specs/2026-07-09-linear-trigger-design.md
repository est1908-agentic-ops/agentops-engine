...
Status: implemented, then revised same-day (see addendum) · 2026-07-09 · Owner: Artem
...
**Read the [Addendum](#addendum-db-only-registration-2026-07-09) first.** The sections below describe the *first cut*, built against the static `PROJECT_REGISTRY_JSON` registry (per the Non-goals section's now-incorrect claim that Linear support would be static-registry-only). That registry was deleted entirely the same day, before merge — `agentops-platform` never used it, DB (`managed_projects`) was already the real mechanism for GitHub projects. Everything below describing `project-registry.ts`, `PROJECT_REGISTRY_JSON`, `linearTokenEnvVar`, or the chart's `projects` map is historical — read it for the *problems* (label-by-ID, ref-routing) which are still solved the same way, not for the *file paths*, which moved into the managed-project registry. The addendum is the current state.
...
- ~~**DB-managed Linear projects.** `resolveManagedProjectEntry`'s Postgres-backed registry (`docs/superpowers/specs/2026-07-08-managed-project-registry-design.md`) hardcodes `trackerType: 'github'` and is untouched here. Linear projects are static-registry-only (`PROJECT_REGISTRY_JSON`) until a real reason to extend the managed-project schema to a second tracker (and a second encrypted-credential column) shows up.~~ **SUPERSEDED same-day, before merge — see the [Addendum](#addendum-db-only-registration-2026-07-09).** The "real reason" showed up immediately: the static registry this bullet assumed would keep existing was deleted entirely, and Linear support moved into the managed-project registry.
...
## Addendum: DB-only registration (2026-07-09)

Same day, before this PR merged: confirmed with Artem that `agentops-platform` doesn't use the chart's static `projects:` values map at all anymore — every project (there and here) is registered through the DB-backed managed project registry (`managed_projects` table, [managed-project-registry-design.md](2026-07-08-managed-project-registry-design.md)), which had already superseded the static registry for GitHub projects. The design above built Linear support against the static registry specifically *because* that registry's own design doc named "tracker config/credentials beyond today's single GitHub credential" as a non-goal reserved for "if a real second tracker... is ever built" — that point arrived immediately, same day, before merge. Rather than ship dead code (a static-registry Linear path nobody would use) and delete it in a follow-up PR, this addendum folds Linear support directly into the managed project registry and removes the static registry from the engine entirely — not just from Linear's code path, but the whole mechanism, including the working GitHub path that predates this PR.

### What actually changed

**Schema (`packages/activities/src/postgres-managed-project-store.ts`).** `managed_projects` gains four additive columns, following the managed-project design's own "additive-only, `ADD COLUMN IF NOT EXISTS`" policy (§4.1): `tracker_type TEXT NOT NULL DEFAULT 'github'`, `encrypted_linear_token TEXT`, `linear_team_key TEXT`, `linear_trigger_label_id TEXT`, plus a partial unique index on `linear_team_key` (`WHERE linear_team_key IS NOT NULL`, so NULLs on every github row never collide). One keypair encrypts both credentials — no second keypair needed, asymmetric encryption doesn't require a matching key per secret.

**Contracts.** `ManagedProjectSchema`/`CreateManagedProjectRequestSchema`/`UpdateManagedProjectRequestSchema` (`packages/contracts/src/{managed-project,control-projects-api}.ts`) grow `trackerType`/`linearTeamKey`/`linearTriggerLabelId`/`linearCredentialSet` fields, `trackerType` immutable once a project exists (like `project`/`repo` already were) — changing tracker means delete + recreate. `packages/contracts/src/project-registry.ts` (the static-registry zod schema, `parseProjectRegistry`, `loadProjectRegistry`) is deleted outright; `ResolvedProjectEntry` moves to a new `resolved-project-entry.ts` as a plain (non-zod) discriminated type — it was always "constructed programmatically, never parsed from raw input" per its own comment, and now that's true without exception, so the `tokenEnvVar`/`linearTokenEnvVar` fields (which only ever mattered for the env-var-JSON load path) drop out entirely.

**Resolution (`packages/activities/src/resolve-managed-projects.ts`).** `resolveManagedProjectEntry` drops its `staticRegistry` fallback parameter — DB miss is just `null` now, nothing to fall through to. A new `resolveManagedProjectEntryByLinearTeamKey` (backed by a new `PostgresManagedProjectStore.getByLinearTeamKey`) is the DB equivalent of the deleted `packages/gateway/src/resolve-linear-project.ts`'s static array lookup.

**Gateway/worker/CLI.** All three drop their `registry: ResolvedProjectEntry[]` static-array parameter and the code that built it (`loadProjectRegistry()`, `mergeStaticAndManagedRegistries`). `packages/gateway/src/create-gateway-server.ts`'s Linear route now calls `resolveManagedProjectEntryByLinearTeamKey` instead of the deleted static `findLinearProjectEntry`. `packages/cli`'s `engine start` is DB-only (falls back to in-memory demo mode only when *no* DB is configured at all, same as before, just without the static-registry middle case). `engine project add`/`update` (`packages/cli/src/main.ts`) gain `--tracker-type`, `--linear-team-key`, `--linear-trigger-label-id`, `--linear-token` flags, passed straight through to `control`'s `/api/projects` (`packages/control/src/create-control-server.ts`), which now threads them into `store.upsert()`.

**Chart.** `charts/engine/values.yaml`'s `projects:` map, `_helpers.tpl`'s `engine.projectRegistryJson`, and every `PROJECT_REGISTRY_JSON`/per-project `GITHUB_TOKEN__<PROJECT>`/`LINEAR_TOKEN__<PROJECT>` env var block (worker, gateway, *and* control deployments — control's `PROJECT_REGISTRY_JSON` fed a now-also-deleted `readRegistryRepos` hint-repos picker) are gone. `_helpers.tpl` itself is deleted — it had nothing else in it. `control`'s `/api/registry/repos` hint-repos endpoint now lists repos from the managed-project store instead.

### Named risk

**Mission Control's `ProjectsPage` create/edit form doesn't yet support `trackerType: linear`** — it only has GitHub-shaped fields (`project`, `repo`, `token`, `config`). The backend/CLI fully support Linear-tracked projects; the UI form is a follow-up, not built here. Operators create/update Linear-tracked projects via `engine project add --tracker-type linear --linear-team-key ... --linear-trigger-label-id ... --linear-token ...` (or `update`) until that form is extended.

### Package/file summary (addendum)

- **New:** `packages/contracts/src/resolved-project-entry.ts` — a plain type, no runtime logic, no test file (unlike the zod schema it replaces).
