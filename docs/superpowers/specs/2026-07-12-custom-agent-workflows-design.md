...
- **Tier 2** — a project that needs a workflow *shape* no built-in provides authors its own TypeScript workflow using `@agentic-ops/engine-sdk` and runs it in **its own worker**, delegating heavy activities back to the engine.
...
| **2. Per-project worker** | **custom workflow structure in TS**, using `@agentic-ops/engine-sdk`; heavy activities delegated to the engine | its own worker image + deploy in its namespace | never (project deploys its own worker) |
...
## 4. Tier 2 — `@agentic-ops/engine-sdk` + per-project worker
...
| 2 | Tier 2: SDK + per-project worker + authz | Publish `@agentic-ops/engine-sdk` (§4.1); reference per-project worker + task-queue routing + `createEngineWorker` (§4.2); the topology + project-identity-binding authz (§4.3); `"continuous"` agents in the reconciler. |
