...
- A second reconcile output — `toStartContinuous` / `toTerminateContinuous` — computed by diffing declared `continuous` agents against the running singletons (listed by workflow-ID prefix `agent:<project>:`).
