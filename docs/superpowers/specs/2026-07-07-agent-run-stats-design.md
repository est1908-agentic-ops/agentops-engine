...
- **`ensureSchema()`'s `CREATE TABLE IF NOT EXISTS` is not atomic across concurrent sessions under READ COMMITTED.** If two worker replicas started simultaneously against a fresh (tableless) database, both can pass the existence check before either commits, and one gets a transient unique-violation (`42P07`) on the catalog. Not a real risk today — `charts/engine/values.yaml`'s `replicas: 1` means only one worker ever runs `ensureSchema()`, and a crash-on-startup self-heals via the pod restarting once the table exists. Flagged so it isn't rediscovered from scratch if this worker is ever scaled past one replica — fix then is a `pg_advisory_lock` around `ensureSchema()` or catching `42P07` specifically.
