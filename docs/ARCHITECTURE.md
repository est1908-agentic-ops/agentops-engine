...
- **Onboarding a new project = one PR** to `agentops-platform` (register Application, namespace, quotas, LiteLLM keys, DNS subzone). An ApplicationSet generator can automate even that from a repo label. Concretely, for forge access: a **DB-backed managed project registry** (`managed_projects` table, credentials encrypted asymmetrically — `control` can write but not read them) replaces the single global `GITHUB_TOKEN` this repo shipped with through M2, and the chart-driven static registry that briefly replaced *that* — the worker builds one scoped GitHub (or Linear) client per registered project instead of sharing one token across every project. Projects are registered via `engine project add` (a thin client of `control`'s `/api/projects`) or Mission Control, not a values/PR change. See [managed-project-registry-design.md](superpowers/specs/2026-07-08-managed-project-registry-design.md) for the full design, and [the Linear trigger design's DB-only addendum](superpowers/specs/2026-07-09-linear-trigger-design.md) for why the static registry was removed.
